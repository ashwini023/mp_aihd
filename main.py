...
import google.generativeai as ocr
from werkzeug.utils import secure_filename

ocr.configure(api_key='AIzaSyD5UWzyTDVlOtU9tNbCAEscvXxQvk7MVOg')
ocr_model = ocr.GenerativeModel('models/gemini-1.5-flash-002')

def process_image(filepath):
    try:
        sample_file = ocr.upload_file(path=filepath)
        text = "OCR this image"
        response = ocr_model.generate_content([text, sample_file])
